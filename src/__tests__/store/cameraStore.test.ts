...
const createMockCamera = (overrides: Partial<CameraDevice> = {}): CameraDevice => ({
  id: 'cam-001',
  name: '测试设备',
  brand: 'xiaomi',
  model: 'test',
...
describe('CameraStore', () => {
  ...
  describe('selectDevice - 选择设备', () => {
    it('应该选择指定的设备', () => {
      const device = createMockCamera({ id: 'cam-001' });
  ...
  describe('addDevice - 添加设备', () => {
    it('应该添加设备到列表', () => {
      const device = createMockCamera({ id: 'cam-002', name: '新设备' });
  ...
  describe('pairDevice - 配对设备', () => {
    it('应该配对设备并添加到列表', async () => {
      const pairedDevice = createMockCamera({
        id: 'cam-paired',
        name: '配对设备',
        model: 'TEST-MODEL',
