...
const VoiceWave = memo(function VoiceWave({
  isActive,
...
const HeaderMemo = memo(function HeaderMemo({ currentPetName }: { currentPetName: string }) {
  return (
    <header className="sticky top-0 z-40 glass-effect border-b border-slate-200/50 backdrop-blur-xl">
      ...
      <button
        ...
        className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-2xl transition-all duration-500 ${
          isRecording || isAnalyzing
            ...
            : 'bg-gradient-to-r from-orange-500 to-cyan-500 text-white hover:scale-105 active:scale-95'
        }`}
      >
  ...
  return (
    <div className="relative flex justify-center py-12">
      <div className="relative">
        {isRecording && (
          <div className="absolute -inset-20 bg-gradient-to-r from-orange-400/20 to-cyan-400/20 rounded-full animate-ping" />
        )}
...
const FooterInfoMemo = memo(function FooterInfoMemo() {
  return (
    <GlassCard className="text-center space-y-4 pt-5 pb-6">
      <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-400 to-cyan-500 mx-auto flex items-center justify-center shadow-xl shadow-blue-400/40">
...
export const TranslatorPage = memo(function TranslatorPage() {
  ...
  const analyzeVoice = useCallback(() => {
    ...
    analyzeTimeoutRef.current = setTimeout(() => {
      ...
      setTranslation(safeTranslation);
    ...
    return (
      <div className="text-center">
        <div className="inline-flex items-center gap-4 bg-slate-900/95 backdrop-blur-xl px-8 py-4 rounded-full border border-slate-700/60 shadow-2xl">
          <VoiceWave isActive={isRecording} color="from-green-400 to-emerald-500" />
    ...
    return (
      <GlassCard className="space-y-6" glow>
        <div className="flex items-center gap-4">
          <div
            className={`w-20 h-20 rounded-3xl bg-gradient-to-br ${emotionStyle.bg} flex items-center justify-center text-4xl shadow-2xl`}
          >
            {emotionIcon}
          </div>
        ...
        <div
          className={`relative bg-gradient-to-br from-orange-50 to-cyan-50 rounded-2xl p-5 border ${emotionStyle.border}`}
        >
  ...
  }, [showResult, confidence, translation, emotionStyle, emotionIcon, emotionLabel, currentPet, resetResult]);
