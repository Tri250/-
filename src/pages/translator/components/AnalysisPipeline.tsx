// ============================================
// PawSync Pro - AnalysisPipeline.tsx
// 
// 描述: 分析流程编排组件
// ============================================

import React from 'react';
import { Sparkles, Activity, Music2, Volume2, Radio, Drum, Palette } from 'lucide-react';
import { AnalysisAnimation } from '../../../components/animations';
...
export function AnalysisPipeline({
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      {/* 分析动画 */}
      {isActive && (
        <AnalysisAnimation isActive={isActive} source={source} />
      )}
        ...
        <div className="text-center space-y-3 animate-fadeIn">
          <div className="flex items-center justify-center gap-3">
            ...
            <p className="text-gray-600 font-medium text-lg">
              {source === 'image' ? 'AI正在分析图片...' : 'AI正在深度分析...'}
            </p>
            <Sparkles className="w-6 h-6 text-orange-500 animate-pulse" />
          </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          {isActive && (
            <Badge color="orange" size="small" className="animate-pulse">
              分析中
            </Badge>
          )}
        </div>
